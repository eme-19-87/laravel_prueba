...
<section class="bg-white">
  ...
  <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16">
    <h1 class="text-black text-3xl font-extrabold mt-10 mb-10">Posts y Anuncios</h1>
    ...
    @foreach ($posts as $post)
        ...
        @if($post->image)
            <article style="background-image: url('{{ url(Storage::url($post->image->url)) }}')" class="bg-black/20 border border-gray-200 rounded-lg p-8 md:p-12 @if($loop->first) col-span-2 mb-8 @endif">
                ...
                <a href="/posts/{{$post->id}}" class="inline-flex justify-center items-center py-2.5 px-5 text-base font-medium text-center text-white rounded-lg bg-white hover:bg-blue-400 hover:text-white focus:ring-4 focus:ring-blue-200">
                  Read more
